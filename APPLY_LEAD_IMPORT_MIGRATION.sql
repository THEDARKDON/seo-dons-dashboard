-- ========================================
-- APPLY MIGRATION 023: Lead Import System
-- This migration adds tables for bulk lead import tracking
-- ========================================

-- Check if lead_imports table already exists
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'lead_imports') THEN
        RAISE NOTICE 'Creating lead_imports table...';
    ELSE
        RAISE NOTICE 'lead_imports table already exists';
    END IF;
END $$;
...
WHERE schemaname = 'public'
  AND tablename IN ('lead_imports', 'lead_import_results')
ORDER BY tablename;

-- Check columns in lead_imports
