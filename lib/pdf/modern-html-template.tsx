...
function renderCompetitionTable(metrics: any[], competitors: any[]): string {
  // Try to get competitor names from multiple sources:
  // 1. From research.enhancedResearch.competitors array (real data)
  // 2. From the metrics data itself (generated by Claude)
  // 3. Fall back to generic labels

  let comp1 = competitors[0]?.name || competitors[0]?.domain;
  let comp2 = competitors[1]?.name || competitors[1]?.domain;
  let comp3 = competitors[2]?.name || competitors[2]?.domain;

  // If we don't have real competitor names, look through ALL metrics to find domain names
  // This handles cases where the first metric row might have numbers but later rows have domains
  if (!comp1 && metrics && metrics.length > 0) {
    for (const row of metrics) {
      const comp1Data = row.competitor1 || row.topCompetitorA || '';
      // Check if it looks like a domain name (contains .co.uk, .com, etc.)
      if (typeof comp1Data === 'string' && comp1Data.includes('.') && !comp1Data.match(/^\d/)) {
        comp1 = comp1Data.replace(/^https?:\/\//, '').replace(/^www\./, '').split('/')[0];
        break;
      }
    }
  }

  if (!comp2 && metrics && metrics.length > 0) {
    for (const row of metrics) {
      const comp2Data = row.competitor2 || row.topCompetitorB || '';
      if (typeof comp2Data === 'string' && comp2Data.includes('.') && !comp2Data.match(/^\d/)) {
        comp2 = comp2Data.replace(/^https?:\/\//, '').replace(/^www\./, '').split('/')[0];
        break;
      }
    }
  }

  if (!comp3 && metrics && metrics.length > 0) {
    for (const row of metrics) {
      const comp3Data = row.leader || row.marketLeader || '';
      if (typeof comp3Data === 'string' && comp3Data.includes('.') && !comp3Data.match(/^\d/)) {
        comp3 = comp3Data.replace(/^https?:\/\//, '').replace(/^www\./, '').split('/')[0];
        break;
      }
    }
  }
  ...
  comp1 = comp1 || 'Competitor 1';
  comp2 = comp2 || 'Competitor 2';
  comp3 = comp3 || 'Competitor 3';
...
function renderPackageComparison(packages: any[]): string {
  // Sort packages by tier (local, regional, national)
  const tierOrder = { 'local': 1, 'regional': 2, 'national': 3 };
  ...
  <div class="card p-6 sm:p-8 mb-6 animate-on-scroll">
    <h3 class="text-lg sm:text-xl font-semibold mb-6">Package Comparison</h3>
    <div class="overflow-x-auto">
      <table class="w-full min-w-[600px]">
        <thead>
          <tr style="border-bottom: 2px solid var(--border);">
            <th class="text-left py-3 px-3 sm:px-4 font-semibold text-xs sm:text-sm">Feature</th>
            ${sortedPackages.map((pkg: any) => `
              <th class="text-center py-3 px-3 sm:px-4 font-semibold text-xs sm:text-sm">
                <div class="mb-2">${escapeHTML(pkg.name)}</div>
                ${pkg.recommended ? '<span class="badge text-xs" style="background-color: var(--accent); color: var(--accent-foreground);">Recommended</span>' : ''}
              </th>
            `).join('')}
          </tr>
        </thead>
        <tbody class="text-xs sm:text-sm">
          <tr style="border-bottom: 1px solid var(--border);">
            <td class="py-3 px-3 sm:px-4 font-medium">Monthly Investment</td>
            ${sortedPackages.map((pkg: any) => {
              const pricing = pkg.pricing?.monthly || pkg.monthlyInvestment || pkg.price || 0;
              return `<td class="py-3 px-3 sm:px-4 text-center font-bold" style="color: var(--accent);">£${pricing.toLocaleString()}</td>`;
            }).join('')}
          </tr>
          <tr style="border-bottom: 1px solid var(--border);">
            <td class="py-3 px-3 sm:px-4 font-medium">Target Keywords</td>
            ${sortedPackages.map((pkg: any) => `<td class="py-3 px-3 sm:px-4 text-center">${pkg.keywordCount || 'N/A'}</td>`).join('')}
          </tr>
          <tr style="border-bottom: 1px solid var(--border);">
            <td class="py-3 px-3 sm:px-4 font-medium">Content Per Month</td>
            ${sortedPackages.map((pkg: any) => `<td class="py-3 px-3 sm:px-4 text-center">${pkg.contentPerMonth || 'N/A'}</td>`).join('')}
          </tr>
          <tr style="border-bottom: 1px solid var(--border);">
            <td class="py-3 px-3 sm:px-4 font-medium">Backlinks Per Month</td>
            ${sortedPackages.map((pkg: any) => `<td class="py-3 px-3 sm:px-4 text-center">${pkg.backlinksPerMonth || 'N/A'}</td>`).join('')}
          </tr>
          <tr>
            <td class="py-3 px-3 sm:px-4 font-medium">Key Deliverables</td>
            ${sortedPackages.map((pkg: any) => {
              const deliverables = pkg.deliverables || pkg.features || [];
              return `
                <td class="py-3 px-3 sm:px-4">
                  <ul class="text-xs space-y-1">
                    ${deliverables.slice(0, 3).map((d: string) => `
                      <li class="flex items-start gap-1">
                        <span style="color: var(--accent);">✓</span>
                        <span>${escapeHTML(d)}</span>
                      </li>
                    `).join('')}
                    ${deliverables.length > 3 ? `<li class="text-xs opacity-75">+${deliverables.length - 3} more</li>` : ''}
                  </ul>
                </td>
              `;
            }).join('')}
